...
export const CatalogLayoutRecyclerPrizesView: FC<CatalogLayoutProps> = () => {
    ...
    if (!levels) {
        return <div className="nitro-catalog-specialized-state" role="status">{LocalizeText('generic.loading')}</div>;
    }

    if (!levels.length) {
        return <div className="nitro-catalog-specialized-state" role="status">{LocalizeText('recycler.info.closed')}</div>;
    }

    return (
        <div className="nitro-catalog-recycler-prizes">
            <h2>{LocalizeText('recycler.prizes.title')}</h2>
            {levels.map((level) => (
                <section key={level.levelId} className="nitro-catalog-recycler-prize-level">
                    <header>
                    ...
                    <div className="nitro-catalog-recycler-prize-grid">
                        {level.products.map((product, index) => {
                            const name = GetFurnitureData(product.productClassId, product.productType)?.name || product.name;

                            return (
                                <div key={`${product.name}-${product.productClassId}-${index}`} className="nitro-catalog-recycler-prize">
