...
const RESULT_KEYS = ['success', 'forbidden', 'invalid_name', 'invalid_url', 'invalid_rank', 'invalid_order', 'not_found', 'persistence_failure'];

export const HousekeepingSoundboardTab: FC = () => {
    const { sounds, lastResult, request, upsert, reorder } = useSoundboardCatalog();
    ...
    const preview = (candidate: SoundboardCatalogDraft) => {
        ...
        void GetSoundManager().playSoundboard(candidate.url.trim());
    };
    ...
    return (
        <div className="flex flex-col gap-2.5">
            ...
            {resultKey && (
                <div role="status" className={`rounded border px-2 py-1 text-xs ${lastResult.resultCode === 0 ? 'border-emerald-300 bg-emerald-50' : 'border-rose-300 bg-rose-50'}`}>
            ...
            <div className="grid grid-cols-2 gap-2 rounded border border-sky-200 bg-sky-50/40 p-2">
                ...
                {!validation.valid && (
                    <div role="alert" className="col-span-2 text-[10px] text-rose-700">
                        {Object.values(validation.errors).map((error) => LocalizeText(`housekeeping.soundboard.validation.${error}`)).join(' · ')}
                    </div>
                )}
                <div className="col-span-2 flex flex-wrap justify-end gap-1.5">
                    ...
                    <button type="button" disabled={!validation.valid} onClick={() => preview(draft)} className="rounded bg-sky-600 px-2 py-1 text-xs font-semibold text-white disabled:opacity-40">
                        {LocalizeText('housekeeping.soundboard.preview')}
                    </button>
                    <button type="button" disabled={!validation.valid} onClick={() => upsert(draft)} className="rounded bg-emerald-600 px-2 py-1 text-xs font-semibold text-white disabled:opacity-40">
                    ...
                    return (
                        ...
                        >
                            <div className="min-w-0">
                                <div className="truncate text-xs font-semibold">{sound.name} <span className="font-normal text-zinc-400">#{sound.id}</span></div>
                                <div className="truncate text-[10px] text-zinc-500">{sound.url} · rank {sound.minRank} · {sound.enabled ? LocalizeText('housekeeping.soundboard.filter.enabled') : LocalizeText('housekeeping.soundboard.filter.disabled')}</div>
                            </div>
                            <div className="flex items-center gap-1">
                                <button type="button" aria-label={`${LocalizeText('housekeeping.soundboard.preview')} ${sound.name}`} onClick={() => preview(sound)} className="rounded bg-sky-100 px-1.5 py-1 text-[10px]">▶</button>
                                <button type="button" aria-label={`${LocalizeText('housekeeping.soundboard.edit')} ${sound.name}`} onClick={() => editSound(sound)} className="rounded bg-zinc-200 px-1.5 py-1 text-[10px]">{LocalizeText('housekeeping.soundboard.edit')}</button>
                                <button type="button" disabled={orderIndex <= 0} aria-label={`${LocalizeText('housekeeping.soundboard.move_up')} ${sound.name}`} onClick={() => moveByOffset(sound.id, -1)} className="rounded bg-zinc-100 px-1.5 py-1 text-[10px] disabled:opacity-30">↑</button>
                                <button type="button" disabled={orderIndex < 0 || orderIndex >= orderedIds.length - 1} aria-label={`${LocalizeText('housekeeping.soundboard.move_down')} ${sound.name}`} onClick={() => moveByOffset(sound.id, 1)} className="rounded bg-zinc-100 px-1.5 py-1 text-[10px] disabled:opacity-30">↓</button>
                            </div>
                        </div>
                    );
                })}
            ...
            <button type="button" disabled={!orderedIds.length} onClick={() => reorder(orderedIds)} className="self-end rounded bg-violet-600 px-2.5 py-1 text-xs font-semibold text-white disabled:opacity-40">
